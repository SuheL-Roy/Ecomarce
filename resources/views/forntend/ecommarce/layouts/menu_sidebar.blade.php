                        ...
                        @foreach ($main_category->related_categories as $category)
                            <li>
                                <a href="{{ route('website_category_products',['main_category'=>$main_category->slug,'category'=>$category->slug]) }}">
                                    ...
                                    @if ($category->related_sub_category->count())
                                        <i class="fa fa-angle-right pull-right"></i>
                                    @endif
                                </a>

                                @if ($category->related_sub_category->count())
                                    <ul style="position: absolute  left: 105%  top: 0; width: 250px; padding: 11px;" >
                                        @foreach ($category->related_sub_category as $sub_category)
                                            <li style="width: 100%; position: relative;">
                                                <a href="{{ route('website_sub_category_products',['main_category'=> $main_category->slug,'category'=> $category->slug,'sub_category'=> $sub_category->slug,]) }}">{{ $sub_category->name }}</a>
                                            </li>
                                        @endforeach
                                    </ul>
                                @endif
